    ...
    return (
        <div className="w-full flex justify-center flex-col gap-4">
            {!previewUrl ? (
                ...
                >
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform ${error ? 'bg-red-100' : 'bg-indigo-50'}`}>
                        <UploadCloud className={`w-6 h-6 ${error ? 'text-red-500' : 'text-indigo-600'}`} />
                    </div>
                    <p className="text-gray-900 font-medium mb-1">Click or drag & drop to upload</p>
